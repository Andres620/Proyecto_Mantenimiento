...
class TST:
        ...
        if (currentNode.isLeaf()):  # if this is a leaf add the node to the left
            ...
            if not currentNode.hasRightChild():  # if this doesnt have right child possibly doesnt have middle child
                if not currentNode.hasMiddleChild():  # doesnt have middle child, lets add
                    ...
                    print(
                        "The node " + str(name) + " has been added as middle child of " + str(currentNode.name))  # show
                    ...
                    print(
                        "The node " + str(name) + " has been added as right child of " + str(currentNode.name))  # show
            else:
    ...
    def adoptChildNode(self, parent, child):
        ...
        if parent.isLeaf():
            ...
            return True
        if not (parent.hasRightChild()):
            if parent.hasMiddleChild():
                parent.middleChild = child
                parent.middleChild.parentNode = parent
            else:
    ...
    def findEmptyChild(self, hijoadop, padreadop, findbool):
        ...
        if findbool:
            ...
            return True
        if padreadop:
            if self.adoptChildNode(padreadop, hijoadop):
                findbool = True
            else:
    ...
    def longerPath(self, root):  # show the bigger path
        if (root):  # root exits
            completPath = self._paths(root, self.arrayPaths,
                                      self.paths)  # get array of paths (root to leafs)( [[5,2,1],[5,2,2]] )
